...
def convert_heic(src, format):
    while True:
        try:
            src = click.prompt(Fore.GREEN + Style.BRIGHT + 'Enter the the source directory for the photos' + Style.RESET_ALL)
            format = click.prompt(Fore.GREEN + Style.BRIGHT + 'Output file format (jpeg, png)' + Style.RESET_ALL)
            ...
            files = [f for f in os.listdir(src) if f.endswith('.HEIC') or f.endswith('.heic')]
